...
/**
 * The <code>VariantVariable</code> provides a decorator for any type of
 * Variable instance, to be able to intercept or monitor variable value
 * modification by using a {@link VariantVariableCallback}. This class will
 * work for implmentations that use {@link #getSyntax()} method to determine the
 * variables syntax. However "instanceof" will not work.
 * 
 * @author Frank Fock
 * @version 1.0
 */
@SuppressWarnings("serial")
public class VariantVariable extends Variable implements AssignableFromInteger,
		AssignableFromLong, AssignableFromString, AssignableFromByteArray {
	 ...
	 *            a <code>Variable</code>.
	 * @param callback
	 *            a callback handler that is called before the value is to be
	 *            modified and after it has been modified.
	 */
	public VariantVariable(Variable initialVariable,
	...
	public synchronized void decodeBER(BERInputStream inputStream)
			throws IOException {
	...
	public void setValue(int value) {
		if (variable instanceof AssignableFromInteger) {
			...
			throw new ClassCastException(
					"An integer value cannot be assigned to " + variable);
		}
	}
	...
	public void setValue(OctetString value) {
		if (variable instanceof AssignableFromByteArray) {
			...
			throw new ClassCastException(
					"An OctetString value cannot be assigned to " + variable);
		}
	}
	...
	public void setValue(byte[] value) {
		if (variable instanceof AssignableFromByteArray) {
			...
			throw new ClassCastException(
					"A byte array value cannot be assigned to " + variable);
		}
	}
	...
	public void setValue(String value) {
		if (variable instanceof AssignableFromString) {
			...
			throw new ClassCastException(
					"A string value cannot be assigned to " + variable);
		}
	}
