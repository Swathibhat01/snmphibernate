...
/**
 * The <code>GenericAddress</code> implements the decorator and factory design
...
public class GenericAddress extends SMIAddress {
	...
	private static final LogAdapter logger = LogFactory
			.getLogger(GenericAddress.class);

	private SMIAddress address;

	private static Hashtable knownAddressTypes = null;
	 ...
	 * types are used by the {@link parse} method to type-safe instantiate
	 * sub-classes from <code>Address</code> from a <code>String</code>.
	 */
	private synchronized static void registerAddressTypes() {
		if (SNMP4JSettings.isExtensibilityEnabled()) {
			...
			try {
				props.load(is);
				Hashtable<String, Class> h = new Hashtable<String, Class>(props.size() + 1);
				...
				String txt = "Could not read '" + addresses + "': "
						+ iox.getMessage();
				logger.error(txt);
			...
			Hashtable<String, Class> h = new Hashtable<String, Class>(3);
			h.put("udp", UdpAddress.class);
			h.put("tcp", TcpAddress.class);
			h.put("ip", IpAddress.class);
			knownAddressTypes = h;
		}
	}
	 ...
	 * 
	 * @param address
	 *            an address string with a leading type specifier as defined in
	 *            the "address.properties". The format is
	 *            <code>"type:address"</code> where the format of
	 *            <code>address</code> depends on <code>type</code>. Valid
	 *            values for <code>type</code> are, for example, "udp" and
	 *            "tcp".
	 * @return a <code>Address</code> instance of the address classes
	 *         specified in "address.properties" whose type ID matched the
	 *         specified ID in <code>address</code>. If <code>address</code>
	 *         cannot be parsed, <code>null</code> is returned.
	...
	public static Address parse(String address) {
		...
		if (c == null) {
			throw new IllegalArgumentException("Address type " + type
					+ " unknown");
		}
		try {
