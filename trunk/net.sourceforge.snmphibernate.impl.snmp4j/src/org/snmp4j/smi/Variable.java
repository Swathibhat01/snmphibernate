...
public abstract class Variable implements Cloneable, Comparable,
	...
	/**
	 * The abstract <code>Variable</code> class serves as the base class for
	 * all specific SNMP syntax types.
	 */
	public Variable() {
	}
	 ...
	 * 
	 * @param inputStream
	 *            an <code>InputStream</code> containing a BER encoded byte
	 *            stream.
	 * @throws IOException
	 ...
	 * 
	 * @param inputStream
	 *            an <code>BERInputStream</code> containing a BER encoded byte
	 *            stream.
	...
	public static Variable createFromBER(BERInputStream inputStream)
		...
		if (SNMP4JSettings.isExtensibilityEnabled()) {
			...
			if (c == null) {
				throw new IOException(
						"Encountered unsupported variable syntax: " + type);
			}
			try {
				variable = (Variable) c.newInstance();
			} catch (IllegalAccessException aex) {
				throw new IOException(
						"Could not access variable syntax class for: "
								+ c.getName());
	...
	/**
	 * Creates a <code>Variable</code> from the supplied SMI syntax
	 * identifier. Subclasses of <code>Variable</code> are registered using
	 * the properties file <code>smisyntaxes.properties</code> in this
	 * package. The properties are read when this method is called first.
	 * 
	 * @param smiSyntax
	 *            an SMI syntax identifier of the registered types, which is
	 *            typically defined by {@link SMIConstants}.
	...
	private synchronized static void registerSyntaxes() {
		...
		try {
			props.load(is);
			Hashtable<Integer, Class> regSyntaxes = new Hashtable<Integer, Class>(props.size());
	 ...
	 *         of <code>Null</code> and its syntax equals one of the
	 *         following:
	 *         <UL>
	 ...
	 *             if an integer representation does not exists for this
	 *             Variable.
	 * @since 1.7
	 */
	 ...
	 *         of Management Information (SMI) modules. '?' is returned if the
	 *         syntax is unknown.
	 * @since 1.7
	 */
	 ...
	 * 
	 * @param impliedLength
	 *            specifies if the sub-index has an implied length. This
	 *            parameter applies to variable length variables only (e.g.
	 *            {@link OctetString} and {@link OID}). For other variables it
	 *            has no effect.
