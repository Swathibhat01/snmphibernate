...
import org.snmp4j.Snmp.PendingRequest;
 ...
 * with the corresponding transport mapping.
 * <p>
...
public class Snmp implements Session, CommandResponder {
  ...
  // The sync object is needed to synchronize sync requests and their responses
   ...
   * <p>
   * As of version 1.1, the supplied message dispatcher is not alterned
   ...
   * <p>
   * The supplied message dispatcher is not alterned
   * in terms of adding any message processing models to it. This has to be
   * done now outside the Snmp class.
   * </p>
   * <p>
      ...
      try {
        if ((!finished) && (retryCount > 0)) {
          try {
            sendMessage(pdu, target, transport);
            if (logger.isDebugEnabled()) {
              logger.debug("Running pending request (" + retryCount +
                           " left) with handle: " + key);
            }
